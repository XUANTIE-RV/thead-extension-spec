...
Earlier versions of this specification and related software may refer to this
extension collection as THEAD or T-HEAD. The collection is now documented under
the XuanTie name. The `XThead*` ISA extension names and the `th.` instruction
and CSR prefixes are retained as architectural identifiers for compatibility
...
* `XTheadVector` provides XuanTie vector instructions.
* `XXTVcoder` provides vector coder turbo instructions.
* `XXTVarith` provides vector arithmetic turbo instructions.
