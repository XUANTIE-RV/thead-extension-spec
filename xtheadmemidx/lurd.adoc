[#xtheadmemidx-insns-lurd,reftext=Load unsigned indexed double-word]
==== th.lurd
...
Mnemonic::
th.lurd _rd_, (_rs1_), _rs2_, _imm2_
...
Description::
This instruction loads a sign extended 64-bit value into GP register _rd_ from the address _rs1_ + (zero_extend(_rs2_) << _imm2_).

Note, that this instruction is equivalent to a `zext.w _rs2_, _rs2_` followed by a `th.lrd` with the same arguments.
